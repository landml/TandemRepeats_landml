package TandemRepeats_landml::TandemRepeats_landmlClient;
...
TandemRepeats_landml::TandemRepeats_landmlClient
...
sub new
{
    ...
    my $self = {
	client => TandemRepeats_landml::TandemRepeats_landmlClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
=head2 tandem_repeats

  $output = $obj->tandem_repeats($params)
...
<pre>
$params is a TandemRepeats_landml.TandemRepeatsParams
$output is a TandemRepeats_landml.TandemRepeatsResults
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function tandem_repeats (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to tandem_repeats:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'tandem_repeats');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "TandemRepeats_landml.tandem_repeats",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method tandem_repeats",
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "TandemRepeats_landml.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "TandemRepeats_landml.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method tandem_repeats",
            status_line => $self->{client}->status_line,
            method_name => 'tandem_repeats',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for TandemRepeats_landml::TandemRepeats_landmlClient\n";
    }
    if ($sMajor == 0) {
        warn "TandemRepeats_landml::TandemRepeats_landmlClient version is $svr_version. API subject to change.\n";
    }
}
...
package TandemRepeats_landml::TandemRepeats_landmlClient::RpcClient;
